...
The `db_qaqc_suite()` runs a series of checks on the Result and Visit tables in the database. These checks are intended to find invalid values in each column of these tables. Three types of checks include lower/upper case consistency, the presence of empty strings (e.g. `""`), and untrimmed strings (e.g. `"FD "`). Because Access does not enforce consistency across these issues, they can cause issues when importing the results to R, which does require consistency. This function will report which columns failed, the reason, and the number of rows that failed each check. The specific rows that failed can be shown by setting `print.values=TRUE`. 
