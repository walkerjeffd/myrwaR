...
# OR EXTRACT FROM METADATA ABOVE AUTOMATICALLY (for compiling document)
...
The Mystic River Watershed Association conducted dry-weather monitoring in the municipalities of Somerville and Melrose on `r rmarkdown::metadata$sampling_date`. The main goal of the monitoring event was to evaluate conditions at stormwater outfalls and other sites that have not been sampled extensively in the past few years by MyRWA and other sites of interest. Water quality samples were taken in accordance with the MyRWA MA-DEP approved QAPP along with accurate notes.
...
wq <- myrwaR::load_wq(path='H:/Dropbox/Work/mystic/_share_db/MysticDB_20141226.accdb',
...
precip <- myrwaR::load_precip_from_xls(path='H:/Dropbox/Work/mystic/_share_db/Processed/Precip/LoganPrecip.xlsx', as.type='dataframe')
```
...
```{r map, fig.width=7, fig.height=7}
map <- get_map(location=c(mean(range(locations$Longitude)), 
...
for (i in 1:nrow(locations)) {
  ...
  plot <- ggmap(map, extent='device', darken=c(0.2, 'black')) +
    ...
    geom_text(aes(x=Longitude, y=Latitude, label=LocationID, color=Current, alpha=Current), 
              data=locations_current,
...
```{r map-waterbody, fig.width=6, fig.height=4.5, fig.align='center'}
# adjust zoom levels to ensure all locations are included on map
waterbody_zoom <- c("Alewife Brook"=14, 
                    "Ell Pond"=16, 
                    "Malden Canal"=14,
                    "Horn Pond"=15, 
                    "Horn Pond Brook"=16, 
                    "Shaker Glen Brook"=15,
                    "Sweetwater Brook"=14,
                    "Wellington Brook"=14)

for (waterbody in sort(unique(locations$WaterBodyID))) {
  # check that zoom level is defined above for this waterbody
  stopifnot(waterbody %in% names(waterbody_zoom))
  ...
  map <- get_map(location=c(mean(range(wq_waterbody$Longitude)),
                            mean(range(wq_waterbody$Latitude))),
                 maptype='hybrid',
                 zoom=waterbody_zoom[[waterbody]])
